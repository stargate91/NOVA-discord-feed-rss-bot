...
import { MonitorConfig, PlatformMetadata } from '@/types/monitor';
...
export default function EditMonitorModal({ 
  ...
  useEffect(() => {
    if (monitor && isOpen) {
      ...
      if (monitor.type === 'crypto') {
        ...
        const pairs = symbolsStr.split(',').filter((p: string) => p.includes(':')).map((p: string) => {
          const [s, t] = p.split(':');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const updateData: Partial<MonitorConfig> & Record<string, any> = {
      name: formData.name,
