...
import { getDashboardTierMeta, resolveClientTierContext } from "@/utils/tier_limits";
import { sanitizeGuildId } from "@/utils/discord";
...
export async function getGuildDashboardStats(guildId: string | number, session: any): Promise<GuildDashboardStats | null> {
  ...
  try {
    ...
    const effectiveIsMaster = isMasterGuild(cleanId) || dbIsMaster || session?.user?.role === "master";

    const features = resolveGuildFeatures(cleanId, tier, effectiveIsMaster, premiumUntil);
    const tierContext = resolveClientTierContext({
      features,
      tier,
    ...
    const isLifetime = Boolean(tierContext.isMaster || (premiumUntil && new Date(premiumUntil) > new Date('2090-01-01')));
    ...
    const fallbackTierContext = resolveClientTierContext({
    ...
    return {
      ...
      totalPosts: 0,
      isPremium: fallbackTierContext.isPremium,
      isLifetime: fallbackTierContext.isMaster,
      maxMonitors: fallbackTierContext.maxMonitors,
      tier: fallbackTierContext.effectiveTier,
      tierName: fallbackTierContext.tierName,
      viewType: `Guild ${cleanId}`,
      features: fallbackFeatures,
      error: error?.message,
    };
