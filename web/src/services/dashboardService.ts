...
const dashboardService = {
  ...
  async getGuildStats(guildId: string | number, session: any): Promise<GuildDashboardStats | null> {
    ...
    try {
      ...
      const monitorsRes = await pool.query('SELECT COUNT(*) FROM monitors WHERE guild_id = $1::bigint AND enabled = true', [cleanId]);
      const totalMonitorsRes = await pool.query('SELECT COUNT(*) FROM monitors WHERE guild_id = $1::bigint', [cleanId]);
      const statsRes = await pool.query('SELECT SUM(post_count) FROM monitor_stats_daily WHERE guild_id = $1::bigint', [cleanId]);
      const guildInfo = await pool.query('SELECT premium_until, tier FROM guild_settings WHERE guild_id = $1::bigint', [cleanId]);
