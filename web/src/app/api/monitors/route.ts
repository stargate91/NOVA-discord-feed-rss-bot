...
export async function GET(request: NextRequest) {
  ...
  } else if ((session.user as any)?.role !== "master") {
  ...
  try {
    ...
    const monitors = res.rows.map(row => {
      ...
      const m: Record<string, any> = { ...row, ...extra };
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const countRes = await pool.query('SELECT COUNT(*) FROM monitors WHERE guild_id = $1::bigint', [guildId]);
