...
export async function POST(request: NextRequest) {
  ...
  const isStaff = (session.user as any)?.role === "master";
  ...
  if (!isStaff) {
    ...
    try {
      const guildCheck = await pool.query(
        'SELECT tier, premium_until FROM guild_settings WHERE guild_id = $1::bigint',
        [cleanId]
      );
  ...
  try {
    ...
    if (action === "delete") {
      ...
      let monitorsToUpdate: number[] = [];
      if (monitorIds && monitorIds.length > 0) {
        monitorsToUpdate = monitorIds;
      } else if (guildId) {
        const guildRes = await pool.query("SELECT id FROM monitors WHERE guild_id = $1::bigint", [guildId]);
