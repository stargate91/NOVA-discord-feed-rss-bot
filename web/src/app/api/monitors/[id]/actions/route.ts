...
import { getBotPermissions } from "@/lib/permissions";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const botInfo = await getBotPermissions(guildId, (session.user as any)?.id);
  ...
  // 1. Security check:
  if (!botInfo.is_admin && (session.user as any)?.role !== "master") {
    return NextResponse.json({ error: "Forbidden: You don't have permission for this server" }, { status: 403 });
  }

  // 2. Feature check:
  if (action === "repost" && !botInfo.features.includes("repost") && (session.user as any)?.role !== "master") {
  ...
  const maxPurge = (session.user as any)?.role === "master" ? 100 : (limits.max_purge || 10);
