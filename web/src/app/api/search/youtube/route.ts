...
export async function GET(request: NextRequest) {
  ...
  try {
    if (!isDirectLink) {
      ...
      if (cacheRes.rows.length > 0) {
        ...
        const ageInDays = (new Date().getTime() - new Date(row.updated_at).getTime()) / (1000 * 60 * 60 * 24);
  ...
  try {
    let result: { id: string; title: string; thumbnail: string } | null = null;
